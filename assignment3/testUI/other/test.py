...
class FLayout(Layout, DragNDropWidget):
	...
	def __init__(self, fNameString=None, fArgStrings=[], numArgs=0, palette=False, **kwargs):
		self.neededWidthCache = None
		self.neededHeightCache = None
		self.set_remove_on_drag(False)
		self.fName = None
		self.fArgStrings = fArgStrings
		...
		"""
		if not palette:#pallete blocks dont need to show all the information
			
			#self.becomeBlock()
			pass
		
		else:#for clopies of pallete blocks
			#self.set_remove_on_drag(False)
			pass
		"""
	...
	def becomeBlock(self):
		self.numArgs = max(self.numArgs, len(self.fArgStrings))
		for i in range(self.numArgs):
			try:
				self.fArgs.append(FArgument(self.fArgStrings[i]))
			except:# ValueError:
	...
	def __deepcopy__(self, dumb):
		self.becomeBlock()
		return FLayout(fNameString=self.fNameString, fArgStrings=self.fArgStrings, numArgs=self.numArgs, palette=False)
