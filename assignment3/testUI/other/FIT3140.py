...
class workSpace(BoxLayout):
	...
	def __init__(self, **kwargs):
		...
		for function in fDefinitions:
			funct = FLayout(fNameString=function[0], numArgs=function[1], palette = True)
			#funct.palette = True
			self.add_widget(funct)
...
class FIT3140Ui(BoxLayout):
	def __init__(self, maze, robotController, **kwargs):
		...
		self.workspace = workSpace(orientation="vertical", size_hint=(1, .9))
