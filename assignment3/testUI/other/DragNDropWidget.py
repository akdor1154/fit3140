...
class DragNDropWidget(Widget):
	...
	def on_touch_down(self, touch):
		...
		if self.collide_point(touch.x, touch.y) and self._dragable:
			...
			if abs(touch.time_end - touch.time_start) > 0.2:
				self.touchRelative = self.to_local(touch.x, touch.y, True)
				print(self.touchRelative)
				print((touch.x, touch.y))
				self.dispatch("on_drag_start")
			else:
				print("too short for you")
		else:
			print "didn't collide: I am",self.pos,'('+str(self.width)+'×'+str(self.height)+')',', touch is ',(touch.x, touch.y)
	...
	def on_touch_move(self, touch):
		if self._dragged and self._dragable:
			x = touch.x - self.touchRelative[0]
			y = touch.y - self.touchRelative[1]
	...
	def on_drag_finish(self):
		...
		if self._dragged and self._dragable:
			...
			for obj in self.droppable_zone_objects:
				if obj.collide_point(self.touchUp.x, self.touchUp.y):
					self.lastDroppedZone = obj
					dropped_ok = True
						
			if dropped_ok:
				print "DROP FUNC"
				self.drop_func(self.lastDroppedZone)
