...
class FName(Widget):
	def __init__(self, fName, **kwargs):
		...
		with self.canvas:
			...
			self.bg = Rectangle(size=self.size, pos=self.pos)
		self.label = Label(text=self.fName, padding=[-FLayout.blockIndent, 0])
...
class FLayout(Layout, DragNDropWidget):
	...
	def __init__(self, fNameString=None, fArgStrings=[], nArguments=0, rootLayout = None, **kwargs):
	...
	def isRoot(self):
		return self.parent is self.rootLayout
			
	def getRootFLayouts(self):
		return [widget for widget in self.rootLayout.children if widget.__class__ is FLayout]
	...
	def updateDropZones(self):
		...
		FLayout.droppable_zone_objects = [self.rootLayout]+reduce(lambda listOfDropZones, rootFLayout: listOfDropZones+rootFLayout._getDropZones(), self.getRootFLayouts(), [])
